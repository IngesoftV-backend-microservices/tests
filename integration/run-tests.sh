...
if [ ! -f "pytest.ini" ]; then
    print_error "Este script debe ejecutarse desde el directorio integration/"
    exit 1
fi
...
VERBOSE=""
COVERAGE=""
HTML="--html=reports/integration/report.html --self-contained-html"
PARALLEL=""
MARKER=""
KEYWORD=""
NO_HTML=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        -v|--verbose)
            ...
            ;;
        -c|--coverage)
            COVERAGE="--cov=. --cov-report=html:reports/integration/coverage --cov-report=term-missing"
            shift
            ;;
        -h|--html)
            HTML="--html=reports/integration/report.html --self-contained-html"
...
PYTEST_CMD="python3 -m pytest integration/"
...
if eval $PYTEST_CMD; then
    ...
    if [ -n "$HTML" ] && [ -z "$NO_HTML" ]; then
        print_info "Reporte HTML generado en: reports/integration/report.html"
    fi
    
    if [ -n "$COVERAGE" ]; then
        print_info "Reporte de cobertura generado en: reports/integration/coverage/index.html"
    fi
    ...
    if [ -n "$HTML" ] && [ -z "$NO_HTML" ]; then
        print_info "Revisa el reporte HTML en: reports/integration/report.html"
    fi
